...
load(file = here::here("input/data/original/survey-edumer/study3_rawdata.RData"))
...
# Define la zona horaria si corresponde (Santiago)
tz_chile <- "America/Santiago"

etapa2 %>% select(session,start_2,ended_2,modified_2,expired_2) %>% skimr::skim()

times <- etapa2 %>%
  mutate(
    # Convertir a POSIXct si son character
    created_dt = ymd_hms(start_2, tz = tz_chile),
    ended_dt   = ymd_hms(modified_2,   tz = tz_chile),

    # Diferencia en segundos
    timesec = as.numeric(difftime(ended_dt, created_dt, units = "secs")),
    # Diferencia en minutos
    timemin = as.numeric(difftime(ended_dt, created_dt, units = "mins"))    
  ) %>% select(session,timesec,timemin)

summary(times$timemin)

df_inicio <- left_join(x = df_inicio,y= times %>% select(session,timemin,timesec),by="session")
```
...
df_individuals<- df_individuals%>% select(pid=session,timemin,timesec,educat,isced,ideology_collapsed,income_ter,income_cua,income_qui,income_eq,
                                 migration,religion,mrat,timemin,timesec,
...
df_vig_long_combined$norelig <- factor(df_vig_long_combined$vrelig,levels = 1:4,c("Catholic","Evangelical","Jewish","Non-religious"))
df_vig_long_combined$norelig <- relevel(factor(df_vig_long_combined$norelig),ref = "Non-religious")
...
df_vig<- df_vig %>% 
  left_join(df_inicio %>% select(pid=session,timemin,timesec,female,age_group,educat,isced,ideology_collapsed,income_ter,income_cua,income_qui,income_eq,
                                 migration,religion,
                                 self_transcendence, self_enhancement, openness_change, conservation,
                                 starts_with("swval")),
            by = "pid")
