...
df_vig<- df_vig %>% sjmisc::row_means("friendship","cooperation","neighborhoodr",n = 3,var = "cohesion",append = T)
  
df_vig <- df_vig %>% group_by(pid) %>% mutate(n_obs=n())

df_vig2 <- 
df_vig %>% select(pid,deck,friendship_z,cooperation,neighborhood,vincome,vedu,norelig,vmigback,polcenter,selftrans,vgender,vage,vempstat,
                  income_qui,educat,migration,religions,ideology_collapsed,humanvalue,female,age_group,cohesion,n_obs,timemin,timesec) %>% na.omit()
...
lm_fship_vig_resp2_t   <- lmer(friendship_z~
                            vincome+vedu+
                              ...
                              ideology_collapsed+humanvalue+female+age_group+scale(timemin)+
                              (1|pid),data = df_vig2)
screenreg(list(lm_fship_vig_resp,lm_fship_vig_resp_t))
...
coef_data$coef_lab<- paste0(round(coef_data$estimate,2)," (",round(coef_data$std.error,2),")")
...
df_plot_valu %>% 
  ...
  labs(caption = paste0("Note: N (observations) = ",nobs(cl_trust_vals),"; N (individuals) = ",lme4::ngrps(cl_trust_vals),". Predictive estimates with 95% confidence intervals"),
       x="Human Values (Vignette)",y="Trust",
